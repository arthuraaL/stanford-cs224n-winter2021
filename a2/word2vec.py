...
def naiveSoftmaxLossAndGradient(
    ...
    outsideSoftmaxProbVec = softmax(outsideVectors.dot(centerWordVec))
    loss = -np.log(outsideSoftmaxProbVec[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + outsideVectors.T.dot(outsideSoftmaxProbVec)
    gradOutsideVecs = outsideSoftmaxProbVec[:, np.newaxis].dot(centerWordVec[:, np.newaxis].T)
    gradOutsideVecs[outsideWordIdx] = centerWordVec.dot(outsideSoftmaxProbVec[outsideWordIdx] - 1)
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec))) - \
        np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec))))  # float

    u_k = outsideVectors[negSampleWordIndices]  # K,d
    u_o = outsideVectors[outsideWordIdx]  # d

    tmp = u_k.T * (1 - sigmoid(-u_k @ centerWordVec))  # d,K
    gradCenterVec = -u_o * (1 - sigmoid(u_o @ centerWordVec)) + np.sum(tmp, axis=1)  # d

    gradOutsideVecs = np.zeros(outsideVectors.shape)  # K,d
    gradOutsideVecs[outsideWordIdx] = (sigmoid(u_o @ centerWordVec) - 1) * centerWordVec  # d
    # check for words sampled multiple times
    for i, val in enumerate(negSampleWordIndices):
        gradOutsideVecs[val] += (1 - sigmoid(-u_k[i].dot(centerWordVec))) * centerWordVec  # d
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    # get the vector for the current center word
    idx_center_word = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[idx_center_word]

    # we desire our probability vector generated to match the true probabilities
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        loss_tmp, gradVc, gradU = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += loss_tmp
        gradCenterVecs[idx_center_word] += gradVc
